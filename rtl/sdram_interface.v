...
*                该模块将xx_busy置低，读写请求可以同时发生。仅支持16bits
...
*                      是效率较低)。该模块不含FIFO。
*                注意：请不要轻易减小SDRAM频率（建议100Mhz或更高）；更小的频率将导致连续读写
*                      占用时间超过刷新周期。
*                clk leads the 80 degree of sdram_clk(Different results
*                for different boards).Recommended sdram_clk phase shift -80 degrees.
*                Note:wr_xxx rd_xxx independent.
*                wr_data no pre-fetch need!
*  Origin       :190119
*                190122
*                190124 - add init_done
*                   |___190124 sdram_core.v
*                   |___190330 sdram_interface.v - 这是sdram_core.v的改进版本，add xx_finish
*                                                  使用 xx_finish 代替 xx_busy
*                       190725 - 增加了 CAS 变量在5处地方
*                       190805 - 增加了 wr_mask,写入掩码,置高屏蔽某个字节的写入,与 wr_data 同步
...
parameter   CLK_FREQUENCY   = 100   ,  // (Mhz) note:请不要轻易减小频率；更小的频率将导致连续读写占用时间超过刷新周期
...
(
    ...
    input   wire    [COL_WIDTH    :0]wr_num,        //写入这行的字数(请 <= 2^COL_WIDTH,一般小于等于512)
    input   wire    wr_request,                     //__|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|____
    input   wire    [15:0]wr_data,                  //XXXXXXXX| 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 |XXXX
    output  reg     wr_allow,                       //____|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|_______________
    output  wire    wr_finish,                      //_________________________________________________|~|____
    
    input   wire    [HADDR_WIDTH-1:0]rd_addr,       //{bank_addr,row_addr,col_addr}
    input   wire    [COL_WIDTH    :0]rd_num,        //读取这行的字数(请 <= 2^COL_WIDTH,一般小于等于512)
    input   wire    rd_request,                     //__|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|____
    output  reg     [15:0]rd_data,                  //XXXX| 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 |XXXX
    output  reg     rd_allow,                       //____|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|_______________
    output  wire    rd_finish,                      //_________________________________________________|~|____
...
wire    wr_busy = wr_flag[0];
wire    rd_busy = rd_flag[0];
reg     [15:0]dly_cnt; 
reg     [7 :0]ck_cnt; 
...
assign  {sdram_dqmh,sdram_dqml}=(data_output)?wr_mask:2'd0;

assign  init_done   =   ~state[4];
wire    finish_     =   state == TRFC3 && ck_cnt == CK_TRFC;
assign  wr_finish   =   (finish_ && wr_busy && (wr_flag[1] == 0))?1'd1:1'd0;//!
assign  rd_finish   =   (finish_ && rd_busy && (rd_flag[1] == 0))?1'd1:1'd0;//!
/*
*   refresh counter
*/  
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        dly_cnt     <=  'd0;
    end else if(nxt_state == TDLY) begin
...
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        ...
        case(nxt_state)
        TRP,TRFC1,TRFC2,TMRD,TRP2,TRFC3,TRCD1:ck_cnt <=  ck_cnt + 1'd1;
//        INIT_NOP1,REFRESH1,REFRESH2,LMR,IDLE,PRECHARGE2,REFRESH3,ACTIVE1,READ:  ck_cnt <= 'd0;
        default:    ck_cnt <= 'd0;
        endcase
    end
end
always@(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
        ...
        case(nxt_state)
        RD_WORD,WR_WORD: word_cnt <=  word_cnt + 1'd1;
//        IDLE:            word_cnt <= 'd0;
        default:    word_cnt <= 'd0;
        endcase
    end
...
//        end
        ...
        LMR:begin                //--->
            BA_A        <= {{BANK_WIDTH{1'b0}},{(ROW_WIDTH-10){1'b0}},{1'b0,2'b00,CAS,4'b0111}};//Programmed Burst Length,full page,CAS 3
            sdram_cmd   <=  CMD_LMR;        
        end                      //   |
        TMRD:begin               //<--+
            sdram_cmd   <=  CMD_NOP;        
...
//READ //------------------------------------------------------------------------
        ...
        RD_WORD:begin
            ...
            //make rd_allow
            if(word_cnt == CAS)//***caution!!***
                rd_allow <= 1'd1;
            else if(word_cnt == CAS + rd_num_b)//***caution!!***
                rd_allow <= 1'd0;
            else
                rd_allow <= rd_allow;
...
//READ //------------------------------------------------------------------------进入前请确保已经失选(disactive),计数器word_cnt为0
        ...
        RD_WORD:begin//原本应是3个CAS NOP然后读512字、发BSTP。这里直接与后边合并//读写完毕后也要刷新
            nxt_state = (word_cnt == rd_num_b + CAS + 2)?PRECHARGE2:RD_WORD;//RD_WORD持续rd_num + 3(CAS==3) + 2个周期,最后5个是NOP,2:可以增大也可为0
            nxt_rd_flag[1] = 1'd0;
        end
